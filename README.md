...
- Веб-симулятор чата без Telegram на `/` (статическая страница), бэкенд-эндпоинт `/simulate` прогоняет ту же бизнес-логику.
...
- Интерфейс: поле ChatId, Username, окно сообщений и кнопок из сценария. Сообщения отправляются на `/simulate`.
